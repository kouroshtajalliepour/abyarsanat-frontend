<template>
  <div class="empty-shopping-cart-container">
    <div class="border-slate-300 border px-6 py-6 flex flex-col items-center">
      <div class="empty-shopping-cart-image">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
          height="800px"
          width="800px"
          version="1.1"
          id="Capa_1"
          viewBox="0 0 512 512"
          xml:space="preserve"
        >
          <g>
            <g>
              <path
                style="fill: #ff4f19"
                d="M490.008,244.345c-4.281-1.117-8.648,1.461-9.75,5.734l-0.521,2.008H32.263l-0.521-2.008    c-1.102-4.281-5.477-6.848-9.75-5.734c-4.273,1.105-6.844,5.473-5.734,9.75l49.789,192.031    c4.57,17.641,20.484,29.961,38.711,29.961h302.484c18.227,0,34.141-12.32,38.711-29.961l49.789-192.031    C496.852,249.818,494.281,245.451,490.008,244.345z M148,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z M188,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z M228,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z M268,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z M308,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z M348,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z M388,428.088c0,6.629-5.375,12-12,12s-12-5.371-12-12v-136    c0-6.629,5.375-12,12-12s12,5.371,12,12V428.088z"
              />
            </g>
            <g>
              <path
                style="fill: #e7001e"
                d="M496,220.088H16c-8.836,0-16,7.163-16,16c0,8.836,7.164,16,16,16h480c8.836,0,16-7.164,16-16    C512,227.251,504.836,220.088,496,220.088z"
              />
            </g>
            <g>
              <path
                style="fill: #8a8895"
                d="M282.275,37.134L282.275,37.134c-8.164,3.382-12.041,12.741-8.659,20.905l76.306,184.218    c3.382,8.164,12.741,12.041,20.905,8.659l0,0c8.164-3.382,12.041-12.741,8.659-20.905L303.18,45.794    C299.798,37.63,290.439,33.753,282.275,37.134z"
              />
            </g>
            <g>
              <path
                style="fill: #8a8895"
                d="M230.119,37.134L230.119,37.134c8.164,3.382,12.041,12.741,8.659,20.905l-76.306,184.218    c-3.382,8.164-12.741,12.041-20.905,8.659l0,0c-8.164-3.382-12.041-12.741-8.659-20.905l76.306-184.218    C212.595,37.63,221.955,33.753,230.119,37.134z"
              />
            </g>
          </g>
        </svg>
      </div>
      <div class="empty-shopping-cart-content">
        <h2>{{ t("pages.cart.emptyCart") }}</h2>
        <!-- <NuxtLink :to="localePath('/product')">
                <p>{{ t("pages.cart.showMoreProducts") }}</p>
            </NuxtLink> -->
      </div>
    </div>
  </div>
</template>

<script lang="ts" setup>
const { t } = useI18n();
const localePath = useLocalePath();
</script>

<style lang="scss">
.empty-shopping-cart-container {
  padding-top: 10px;
  @include lg {
    padding: 24px 24px 0;
    margin: 0 48px;
  }
}
.empty-shopping-cart-image {
  width: 100%;
  display: flex;
  justify-content: center;
  svg {
    width: 150px;
    height: 150px;
  }
}
.empty-shopping-cart-content {
  margin-top: 30px;
  display: flex;
  flex-direction: column;
  align-items: center;
  h2 {
    color: $gray-500;
    margin-top: 10px;
  }
  p {
    color: $title-color;
    margin-top: 10px;
  }
}
</style>
